...
pub(crate) use helpers::{do_finalize_with_seed, read_raffle};
...
if config.randomness_source == RandomnessSource::External {
    ...
    pub fn buy_tickets(env: Env, buyer: Address, quantity: u32) -> Result<u32, Error> {
        ...
        enforce_max_tickets_per_address(&env, &raffle, &buyer, quantity)?;
    ...
    pub fn buy_tickets_for(
        ...
        let raffle = read_raffle(&env)?;
        enforce_max_tickets_per_address(&env, &raffle, &recipient, quantity)?;
